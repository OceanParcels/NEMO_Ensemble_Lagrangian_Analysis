...
ax.scatter(pset_space.lon[indexes_space, 0], pset_space.lat[indexes_space, 0],
                   s=10, color='b', alpha=1, label='Varying Space')

# Scatter plot for varying time particle
ax.scatter(pset_temp.lon[0, 0], pset_temp.lat[0, 0],
                   s=20, color='r', alpha=1, marker='s', label='Varying Time', zorder=10)

# Plot trajectories for varying space particles
ax.plot(pset_space.lon[indexes_space, :t].T, pset_space.lat[indexes_space, :t].T, c='b', 
                ls='-', alpha=0.3)

# Plot trajectories for varying time particle
ax.plot(pset_temp.lon[indexes, :t].T, pset_temp.lat[indexes, :t].T, c='r',
                ls='-', alpha=0.3)
